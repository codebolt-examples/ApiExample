...
// .wrangler/tmp/bundle-hALaqy/checked-fetch.js
...
var parse = /* @__PURE__ */ __name((cookie) => {
...
var Context = /* @__PURE__ */ __name(class {
  constructor(req, env = {}, executionCtx = void 0, notFoundHandler2 = () => new Response()) {
    ...
    this.pretty = (prettyJSON2, space = 2) => {
      this._pretty = prettyJSON2;
...
var getPathFromURL = /* @__PURE__ */ __name((url, strict = true) => {
...
var getQueryStringFromURL = /* @__PURE__ */ __name((url) => {
  const queryIndex = url.indexOf("?");
...
__name(extendRequestPrototype, "extendRequestPrototype");
...
var StaticRouter = /* @__PURE__ */ __name(class {
  constructor() {
    this.middleware = {};
    this.routes = {};
...
var Node2 = /* @__PURE__ */ __name(class {
  ...
  insert(method, path, handler) {
    ...
    const errorMessage = /* @__PURE__ */ __name((name) => {
...
var Hono = /* @__PURE__ */ __name(class extends defineDynamicClass() {
  constructor(init = {}) {
    super();
...
// src/routes/subtract.ts
var subtractRoute = new Hono();
subtractRoute.post(async (c) => {
  const { a, b } = await c.req.json();
  return c.json({ result: a - b });
});

// src/routes/multiply.ts
var multiplyRoute = new Hono();
multiplyRoute.post(async (c) => {
  const { a, b } = await c.req.json();
  return c.json({ result: a * b });
});

// src/routes/divide.ts
var divideRoute = new Hono();
...
// src/routes/modulus.ts
var modulusRoute = new Hono();
...
// src/routes/power.ts
var powerRoute = new Hono();
powerRoute.post(async (c) => {
  const { a, b } = await c.req.json();
  return c.json({ result: Math.pow(a, b) });
});

// src/routes/average.ts
var averageRoute = new Hono();
averageRoute.post(async (c) => {
  const { a, b } = await c.req.json();
  return c.json({ result: (a + b) / 2 });
});

// src/routes/squareRoot.ts
var squareRootRoute = new Hono();
squareRootRoute.post(async (c) => {
  const { a } = await c.req.json();
  if (a < 0)
    return c.json({ error: "Square root of negative number is not allowed." }, 400);
  return c.json({ result: Math.sqrt(a) });
});

// src/middleware/auth.ts
var authMiddleware = new Hono();
...
// .wrangler/tmp/bundle-hALaqy/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-hALaqy/middleware-loader.entry.ts
